...
def spinel_ce(
    ...
    *,
    ...
    if not launch:
        ...
        print(f"A={a_cation} B={b_cation} ce_key={result_key}")
        return result_key
    ...
    if existing_ce is None:
        ...
        _wf_id = lp.add_wf(wf)
        # You said you only want to print A/B and CE key, so we
        # deliberately don't print wf_id here.

    # After launch (or if it already existed), we know the CE key.
    print(f"A={a_cation} B={b_cation} ce_key={ce_key}")
    return ce_key
