...
def _parse_subl_token(v: str) -> SublatticeSpec:
    """
...
def _parse_mix_item(s: str) -> MixtureElement:
    """
    ...
    for p in parts:
        if p.startswith("subl="):
            sublats.append(_parse_subl_token(p[len("subl="):]))
            continue
        if "=" not in p:
...
def main() -> None:
    ...
    mixture: list[MixtureElement] = [_parse_mix_item(s) for s in args.mix]
